  ...
  getStatesFromRedux = () => {
    ...
    let bayans = AuthReducer
      ? AuthReducer.bayans
        ...
        : []
      : [];
    let speecherName = AuthReducer
      ? AuthReducer.speecherName
        ? AuthReducer.speecherName
        : ""
      : "";
    let filter = bayans.filter(v => v.speecherName === speecherName);
    let reverse = _.sortBy(filter, ["timeStamp"]).reverse();
    this.setState({ bayans: reverse, isLoading: false, speecherName });
